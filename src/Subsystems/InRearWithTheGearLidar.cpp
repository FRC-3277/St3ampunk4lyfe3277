#include "InRearWithTheGearLidar.h"
#include "../RobotMap.h"
...
*/
InRearWithTheGearLidar::InRearWithTheGearLidar() : Subsystem("InRearWithTheGearLidar") {
...
============================================================================= */
void InRearWithTheGearLidar::init(int configuration, bool fasti2c, bool showErrorReporting){
	//lumberJack->dLog("InRearWithTheGearLidar::begin");
	...
	hwVersion = readByte(REG_HARDWARE_VERSION, false);
	swVersion = readByte(REG_SOFTWARE_VERSION, false);
...
/* =============================================================================
  hasBegun
	...
	============================================================================= */
bool InRearWithTheGearLidar::hasBegun() {
	//if (logLevel <= VERBOSE) cout << "InRearWithTheGearLidar::hasBegun" << endl;
	if (i2c != nullptr) {
		return true;
	}
	return false;
...
/* =============================================================================
  ...
  - configuration: set the configuration for the sensor
    ...
    - 1 = high speed setting, set the aquisition count to 1/3 the default (works
      great for stronger singles) can be a little noisier
============================================================================= */
void InRearWithTheGearLidar::configure(int configuration){
	//if (logLevel <= VERBOSE) cout << "InRearWithTheGearLidar::configure" << endl;
	bool writeSuccess = false;
  switch (configuration){
    case 0: //  Default configuration
			writeSuccess = i2c->Write((int)0x00, (uint8_t)0x00);
			usleep(1000);
    break;
    case 1: //  Set aquisition count to 1/3 default value, faster reads, slightly
            //  noisier values
			writeSuccess = i2c->Write((int)0x04, (uint8_t)0x00);
			//ofSleepMillis(1);
			usleep(1000);
    break;
    case 2: //  Low noise, low sensitivity: Pulls decision criteria higher
            ...
            //  sensitivity
      writeSuccess = i2c->Write((int)0x1c, (uint8_t)0x20);
			//ofSleepMillis(1);
			usleep(1000);
    break;
    case 3: //  High noise, high sensitivity: Pulls decision criteria into the
      ...
      writeSuccess = i2c->Write((int)0x1c, (int)0x60);
			//ofSleepMillis(1);
			usleep(1000);
    break;
  }
...
/* =============================================================================
  ...
  - faster, but you will need to stabilize DC every once in awhile (ex. 1 out of
...
============================================================================= */
int InRearWithTheGearLidar::distance(bool stablizePreampFlag, bool takeReference){
	//if (logLevel <= VERBOSE) cout << "InRearWithTheGearLidar::distance" << endl;
	int loVal, hiVal, writeSuccess = 0;

  if(stablizePreampFlag){
    // Take acquisition & correlation processing with DC correction
		writeSuccess = i2c->Write(REG_MEASURE, VAL_MEASURE);
		//ofSleepMillis(1);
		usleep(1000);
  }else{
    // Take acquisition & correlation processing without DC correction
		writeSuccess = i2c->Write(REG_MEASURE, VAL_MEASURE_NO_DC_CRCT);
		//ofSleepMillis(1);
		usleep(1000);
  }
...
/* =============================================================================
  ...
  =========================================================================== */
int InRearWithTheGearLidar::signalStrength(){
	if (logLevel <= VERBOSE) cout << "InRearWithTheGearLidar::signalStrength" << endl;
	int sigStrength = readByte(REG_SIGNAL_STRENGTH, false);
	if (sigStrength == -1) return -1;
	else return ((int)((unsigned char) sigStrength));
}

//--------------------------------------------------------------
int InRearWithTheGearLidar::maxNoise(){
	if (logLevel <= VERBOSE) cout << "InRearWithTheGearLidar::maxNoise" << endl;
	int maxNoise = readByte(REG_MAX_NOISE, false);
	if (maxNoise == -1) return -1;
	else return ((int)((unsigned char) maxNoise));
}

//--------------------------------------------------------------
int InRearWithTheGearLidar::correlationPeakValue(){
	if (logLevel <= VERBOSE) cout << "InRearWithTheGearLidar::correlationPeakValue" << endl;
	int corrPeakVal = readByte(REG_CORR_PEAK_VAL, false);
	if (corrPeakVal == -1) return -1;
	else return ((int)((unsigned char) corrPeakVal));
}

//--------------------------------------------------------------
int InRearWithTheGearLidar::transmitPower(){
	if (logLevel <= VERBOSE) cout << "InRearWithTheGearLidar::transmitPower" << endl;
	int transPow = readByte(REG_TRANSMIT_POWER, false);
	if (transPow == -1) return -1;
	else return ((int)((unsigned char) transPow));
}

//--------------------------------------------------------------
int InRearWithTheGearLidar::eyeSafetyOn(){
	if (logLevel <= VERBOSE) cout << "InRearWithTheGearLidar::eyeSafe" << endl;
	int eyeSafety = -1;
...
//--------------------------------------------------------------
int InRearWithTheGearLidar::hardwareVersion() {
	if (logLevel <= VERBOSE) cout << "InRearWithTheGearLidar::hardwareVersion" << endl;
	return hwVersion;
}

//--------------------------------------------------------------
int InRearWithTheGearLidar::softwareVersion() {
	if (logLevel <= VERBOSE) cout << "InRearWithTheGearLidar::hardwareVersion" << endl;
	return swVersion;
}

//--------------------------------------------------------------
int InRearWithTheGearLidar::status() {
	//if (logLevel <= VERBOSE) cout << "InRearWithTheGearLidar::status" << endl;
...
//--------------------------------------------------------------
string InRearWithTheGearLidar::statusString(int statusInt) {
	stringstream out;
...
//--------------------------------------------------------------
int InRearWithTheGearLidar::readByte(int reg, bool monitorBusyFlag) {
	if (logLevel <= VERBOSE) cout << "InRearWithTheGearLidar::readByte" << endl;
	int busyFlag = 0;
    if(monitorBusyFlag){
    busyFlag = 1;
    }
    ...
    if(busyFlag == 0){
		...
		uint8_t output = 0;
		int result = i2c->Read((int)reg, 8, &output);
		if (hardwareVersion() < 21) {
			...
			while (true) {
				if (result == true) {
					usleep(20000);
					result = i2c->Read((int)reg, 8, &output);
...
void InRearWithTheGearLidar::write8Bytes(int registerAddress, int valueToWrite)
{
	uint8_t sendByte;
...
void InRearWithTheGearLidar::InitDefaultCommand() {
